            ...
            <div className="col-md-4">
                <div className="card profile-card-2">
                    ...
                    <div style={{textAlign: "right"}}>
                        <button type="button" className="btn btn-light">{(JSON.parse(this.loggedUser).isAdmin) && <Link to={`/users/edit/${this.props.user.id}`} >Edit</Link>}
                        </button>
                        <button type="button" className="btn btn-primary">{(JSON.parse(this.loggedUser).isAdmin) && <span onClick={() => this.props.onDeleteClick(this.props.user.id)}>Delete</span>}</button>
                    </div>
                    <div className="card-body pt-5">
                        ...
                        <h6 className="card-title">Name : {this.props.user.firstName + " " + this.props.user.lastName}</h6>
                        <p className="card-text">Email: {this.props.user.email}</p>
                        <p>IsAdmin: {this.props.user.isAdmin === true ? 'Yes' : 'No'}</p>
                    </div>
                </div>
            </div>
        )
